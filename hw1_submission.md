...
Increasing the number of threads hurts the performance:
...
(a) There is an improvement in the performance when using 2 threads.
But for some reason I see a decreace in performance when using 4 threads for my 4 cores.
...
(b) There is a 6% improvement when switching from 1 thread to 2
...
(a) The most important seemed to be putting padding between the result addresses

(b) My guess is that if all the counters are stored together will end up in the same cache line.
This line will be duplicated in every private cache because is being read and writen by the different
threads that probably are in different cores. And this line will be continuously be invalidated by other cores.
The problem is the cache coherence of many cores reading and writing to the same cache line.
...
So, if you search the `stats.txt` file for `board.cache_hierarchy.ruby_system.l1_controllers2.L1Dcache.` you should see the number of misses for the second L1 Cache controller's L1 data cache.
...
(a) Algorithm 1 took 0.917 ms, and Algorithm 6 took 0.111 ms

(b) In the real system the difference was more subtle.
...
I tested the Algorithms 2, 3 and 5. Because they implement each unique optimization.

| mem acc   | chunking    | result race   | block race   |
| --------- | ----------- | ------------- | ------------ |
| misses    | 2193        | 4131          | 2160         |
| hits      | 31936       | 27570         | 29863        |
| total     | 34129       | 31701         | 32023        |

The best ratio was obtain by the chunking optimization, with the block race condition opt right behind.
...
Chunking:
           20      3.34%     70.95% |          13      2.17%     73.12% |          13      2.17%     75.29% |          13      2.17%     77.46% |          13      2.17%     79.63% |          14      2.34%     81.97% |          13      2.17%     84.14% |          13      2.17%     86.31% |          13      2.17%     88.48% |          14      2.34%     90.82% |          12      2.00%     92.82% |           8      1.34%     94.16% |          10      1.67%     95.83% |           9      1.50%     97.33% |           8      1.34%     98.66% |           8      1.34%    100.00% (Unspecified)

Result Race:
            376     15.77%     33.00% |          11      0.46%     33.46% |          11      0.46%     33.92% |          12      0.50%     34.42% |          11      0.46%     34.88% |          12      0.50%     35.39% |          12      0.50%     35.89% |          12      0.50%     36.39% |          13      0.55%     36.94% |           8      0.34%     37.27% |           7      0.29%     37.57% |           7      0.29%     37.86% |        1442     60.46%     98.32% |          14      0.59%     98.91% |          11      0.46%     99.37% |          15      0.63%    100.00% (Unspecified)

Block Race:
            1769     73.59%     90.68% |          51      2.12%     92.80% |          13      0.54%     93.34% |          12      0.50%     93.84% |          10      0.42%     94.26% |          15      0.62%     94.88% |          13      0.54%     95.42% |          14      0.58%     96.01% |          13      0.54%     96.55% |           8      0.33%     96.88% |          15      0.62%     97.50% |          12      0.50%     98.00% |          14      0.58%     98.59% |          13      0.54%     99.13% |          15      0.62%     99.75% |           6      0.25%    100.00% (Unspecified)

Definitivelly, chunking has the least read shearing events. So it means that cache blocks are less shared between clocks.
...
Chunking:
        1978      6.02%      6.15% |        2056      6.26%     12.41% |        2061      6.27%     18.68% |        2061      6.27%     24.95% |        2061      6.27%     31.22% |        2061      6.27%     37.49% |        2061      6.27%     43.76% |        2061      6.27%     50.03% |        2061      6.27%     56.31% |        2061      6.27%     62.58% |        2050      6.24%     68.82% |        2050      6.24%     75.05% |        2051      6.24%     81.30% |        2051      6.24%     87.54% |        2052      6.24%     93.78% |        2044      6.22%    100.00% (Unspecified)

Result Race:
        2010      6.13%      6.25% |        2061      6.28%     12.53% |        2061      6.28%     18.81% |        2061      6.28%     25.09% |        2061      6.28%     31.37% |        2061      6.28%     37.65% |        2061      6.28%     43.93% |        2062      6.28%     50.22% |        2061      6.28%     56.50% |        2050      6.25%     62.74% |        2048      6.24%     68.99% |        2040      6.22%     75.20% |        1991      6.07%     81.27% |        2047      6.24%     87.51% |        2063      6.29%     93.80% |        2036      6.20%    100.00% (Unspecified)

Block Race:
            13      6.77%     27.60% |           8      4.17%     31.77% |           6      3.12%     34.90% |           4      2.08%     36.98% |           7      3.65%     40.62% |          14      7.29%     47.92% |           5      2.60%     50.52% |           5      2.60%     53.12% |          16      8.33%     61.46% |           3      1.56%     63.02% |          13      6.77%     69.79% |          14      7.29%     77.08% |          14      7.29%     84.38% |          14      7.29%     91.67% |          14      7.29%     98.96% |           2      1.04%    100.00% (Unspecified)

For sure, the block race optimization has the less events of caches asking to have a block in writable state.
This is because the threads are only reading the data array, but reading and writing to each result address.
Clearly the padding helps to give exclusivity of writing to their result address.
...
Latency for the three algorithms:

|                     | chunking    | result race   | block race   |
| ------------------- | ----------- | ------------- | ------------ |
| missLatenvy avg.    | 975.05      | 334.66        | 65.26        |

(a) Clearly reducing write sharing is the best for the performance.
(b) The cache coherence MESI protocol gets penalized a lot when many cores request
the same cache block to be writable.
...
After running algorithm 1 and 6, with 1 and 25 clocks of bus latency, I got the following sim. times:

|              | naive       | all-opt       |
| ------------ | ----------- | ------------- |
| 1 clock      | 0.349 ms    | 0.102 ms      |
| 25 clock     | 1.760 ms    | 0.127 ms      |

Increasing the latency in the l1 cache memory bus does impact the performance of the naive implementation.
But the effect on the optimized version is lower
